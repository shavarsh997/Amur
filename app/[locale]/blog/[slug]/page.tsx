...
export function generateStaticParams() {
  return locales.flatMap((locale) =>
    getPublishedArticles().map(({ slug }) => ({ locale, slug }))
  );
}
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { label: dictionary.blog.label, href: `/${locale}/blog` },
    { label: copy.title },
  ];
  const articleJsonLd = {
    ...
    mainEntityOfPage: getAbsoluteUrl(`/${locale}/blog/${slug}`),
  ...
  return (
    <>
      ...
      />
      <article>
        <Container className="mx-auto max-w-3xl space-y-10 py-12 text-lg leading-8 text-[var(--text-secondary)] sm:py-16">
          {copy.body.map((section, index) => (
            <section key={`${section.heading ?? "intro"}-${index}`}>
              {section.heading ? (
                <h2 className="mb-4 text-3xl font-semibold tracking-[-0.04em] text-[var(--text-primary)]">
                  {section.heading}
                </h2>
              ) : null}
