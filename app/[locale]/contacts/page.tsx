...
function isReady(value: string) {
  return Boolean(value) && !value.includes("TODO_REPLACE");
}
...
export default async function ContactsPage({ params }: Props) {
  ...
  ].filter((item) => isReady(item.value));
  const messengers = [
        ...
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
          <aside className="space-y-8">
                ...
                <dl className="space-y-6">
                  {contactItems.map((item) => (
                    <div key={item.label}>
                      ...
                      <dd className="mt-1 text-lg text-zinc-950">{item.value}</dd>
                    </div>
                  ))}
                </dl>
              ) : (
                <p className="leading-7 text-zinc-600">{copy.mapPlaceholder}</p>
              )}
              {messengers.length ? (
                <div className="mt-8 border-t border-zinc-200 pt-6">
                  <h2 className="font-semibold text-zinc-950">{copy.messengers}</h2>
            ...
            <div
              ...
              className="grid min-h-72 place-items-center rounded-3xl border border-dashed border-zinc-300 bg-zinc-100 p-8 text-center text-zinc-600"
              role="img"
            >
              {copy.mapPlaceholder}
            </div>
          </aside>
          <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-9" id="estimate">
            <h2 className="text-3xl font-semibold text-zinc-950">{copy.formTitle}</h2>
            <p className="mt-4 leading-7 text-zinc-600">{copy.formDescription}</p>
