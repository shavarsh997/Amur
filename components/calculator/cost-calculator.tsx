  ...
  return <div className={`grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_20rem] ${className}`}>
    <div className="space-y-5">
      ...
      <section className="rounded-[28px] border border-[var(--border)] bg-white p-5 shadow-[var(--shadow-card)] sm:p-7">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><NumberField label={copy.fields.area} min={1} onChange={(area) => update({ area })} value={values.area} /><NumberField label={copy.fields.floors} min={1} onChange={(floors) => update({ floors })} value={values.floors} /><NumberField label={copy.fields.rooms} min={0} onChange={(rooms) => update({ rooms })} value={values.rooms} /><NumberField label={copy.fields.bathrooms} min={0} onChange={(bathrooms) => update({ bathrooms })} value={values.bathrooms} /></div>
        {hasConstruction ? <div className="mt-8 border-t border-[var(--border)] pt-7"><h3 className="text-base font-semibold text-[var(--text-primary)]">{copy.fields.houseParameters}</h3><div className="mt-5 grid gap-5 lg:grid-cols-3"><ChoiceGroup label={copy.fields.houseShape} options={copy.construction.houseShapes} selected={values.houseShape} onChange={(houseShape) => update({ houseShape: houseShape as HouseShape })} /><ChoiceGroup label={copy.fields.constructionType} options={copy.construction.materials} selected={values.material} onChange={(material) => update({ material: material as ConstructionMaterial })} /><div className="space-y-3"><SwitchRow checked={values.basement} label={copy.construction.extras.basement} onChange={(basement) => update({ basement })} />{values.basement ? <NumberField label={copy.fields.basementArea} onChange={(basementArea) => update({ basementArea })} value={values.basementArea} /> : null}</div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><OptionWithArea checked={values.garage} fieldLabel={copy.fields.garageArea} label={copy.construction.extras.garage} value={values.garageArea} onCheckedChange={(garage) => update({ garage })} onValueChange={(garageArea) => update({ garageArea })} /><OptionWithArea checked={values.terrace} fieldLabel={copy.fields.terraceArea} label={copy.construction.extras.terrace} value={values.terraceArea} onCheckedChange={(terrace) => update({ terrace })} onValueChange={(terraceArea) => update({ terraceArea })} /></div></div> : null}
        {hasRenovation ? <div className="mt-8 border-t border-[var(--border)] pt-7"><h3 className="text-base font-semibold text-[var(--text-primary)]">{copy.fields.propertyCondition}</h3><div className="mt-5 grid gap-5 lg:grid-cols-3"><ChoiceGroup label={copy.fields.propertyType} options={copy.renovation.objectTypes} selected={values.renovationObjectType} onChange={(renovationObjectType) => update({ renovationObjectType: renovationObjectType as RenovationObjectType })} /><ChoiceGroup label={copy.fields.currentCondition} options={copy.renovation.conditions} selected={values.renovationCondition} onChange={(renovationCondition) => update({ renovationCondition: renovationCondition as RenovationCondition })} /><ChoiceGroup label={copy.fields.renovationLevel} options={copy.renovation.levels} selected={values.renovationLevel} onChange={(renovationLevel) => update({ renovationLevel: renovationLevel as RenovationLevel })} /></div></div> : null}
      </section>

      {hasConstruction ? <section className="rounded-[28px] border border-[var(--border)] bg-white p-5 shadow-[var(--shadow-card)] sm:p-7"><StepTitle number="03" {...copy.steps.construction} /><div className="mt-6 grid gap-3 md:grid-cols-3">{(Object.keys(config.construction.packages) as ConstructionPackage[]).map((packageKey) => { const option = copy.construction.packages[packageKey]; return <button aria-pressed={values.constructionPackage === packageKey} className={`${selectionClass(values.constructionPackage === packageKey)} min-h-32`} key={packageKey} onClick={() => update({ constructionPackage: packageKey })} type="button"><span className="font-semibold text-[var(--text-primary)]">{option.title}</span><span className="mt-2 block text-xs leading-5 text-[var(--text-secondary)]">{option.description}</span></button>; })}</div></section> : null}
      {hasConstruction ? <section className="rounded-[28px] border border-[var(--border)] bg-white p-5 shadow-[var(--shadow-card)] sm:p-7"><StepTitle number="04" {...copy.steps.constructionExtras} /><div className="mt-6 grid gap-3 sm:grid-cols-2"><SwitchRow checked={values.highCeilings} label={copy.construction.extras.highCeilings} onChange={(highCeilings) => update({ highCeilings })} /><SwitchRow checked={values.difficultSite} label={copy.construction.extras.difficultSite} onChange={(difficultSite) => update({ difficultSite })} /></div><div className="mt-4 max-w-xs"><NumberField hint={copy.fields.distanceHint} label={copy.fields.distance} onChange={(distanceKm) => update({ distanceKm })} value={values.distanceKm} /></div></section> : null}
...
function ChoiceGroup({ label, onChange, options, selected }: { label: string; selected: string; options: Record<string, string>; onChange: (value: string) => void }) {
  return <fieldset><legend className="text-sm font-semibold text-[var(--text-primary)]">{label}</legend><div className="mt-3 space-y-2">{Object.entries(options).map(([value, option]) => <button aria-pressed={selected === value} className={`flex w-full items-center justify-between gap-3 rounded-xl border px-3 py-2.5 text-left text-sm transition ${selected === value ? "border-[var(--text-primary)] bg-[var(--background-warm)]" : "border-[var(--border)] hover:bg-[var(--surface-muted)]"}`} key={value} onClick={() => onChange(value)} type="button"><span>{option}</span>{selected === value ? <Check aria-hidden="true" className="size-4 shrink-0" /> : null}</button>)}</div></fieldset>;
}

function OptionWithArea({ checked, fieldLabel, label, onCheckedChange, onValueChange, value }: { checked: boolean; fieldLabel: string; label: string; value: string; onCheckedChange: (checked: boolean) => void; onValueChange: (value: string) => void }) {
  return <div className="space-y-2"><SwitchRow checked={checked} label={label} onChange={onCheckedChange} />{checked ? <NumberField label={fieldLabel} onChange={onValueChange} value={value} /> : null}</div>;
}
