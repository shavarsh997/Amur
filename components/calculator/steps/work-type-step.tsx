...
export function WorkTypeStep({
  ...
  onChange: (value: CalculatorValues["workType"]) => void;
}) {
  return (
    <section>
      ...
      <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
        {copy.workType.description}
      </p>
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {calculatorConfig.workTypes.map((option) => {
          const Icon = icons[option.value];
          const selected = values.workType === option.value;
          return (
            <button
            ...
            >
              ...
              <span className="font-semibold text-[var(--text-primary)]">
                {copy.workType.choices[option.labelKey]}
              </span>
            </button>
          );
        })}
      ...
      {errors.workType ? (
        <p className="mt-3 text-sm text-[var(--error)]">{errors.workType}</p>
      ) : null}
    </section>
  );
}
