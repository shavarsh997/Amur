...
export function LocationStep({
  ...
  return (
    <section>
      ...
      <label
        className="mt-7 block text-sm font-semibold text-[var(--text-primary)]"
        htmlFor="calculator-region"
      >
        {copy.location.label}
        <input
          aria-invalid={Boolean(errors.location)}
          className="mt-2 min-h-14 w-full rounded-xl border border-[var(--border)] bg-white px-4 text-lg outline-none focus:border-[var(--text-primary)]"
          id="calculator-region"
      ...
      {errors.location ? (
        <p className="mt-3 text-sm text-[var(--error)]">{errors.location}</p>
      ) : null}
    </section>
  );
}
