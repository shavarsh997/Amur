...
export function calculateEstimate(values: CalculatorValues): EstimateResult {
  ...
  const rate = calculatorConfig.pricing.rates.find(
    (item) => item.objectType === values.objectType && item.workType === values.workType,
  );

  if (!rate || !Number.isFinite(area) || area <= 0) {
    return { status: "unavailable", currency: calculatorConfig.pricing.currency };
  }

  const multiplier = values.options.reduce(
    (total, option) => total * calculatorConfig.pricing.optionMultipliers[option],
    1,
  );
